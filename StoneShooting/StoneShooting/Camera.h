...
#define TOP_VIEW_CAMERA 0x01
#define STONE_CAMERA 0x02
...
public:
	XMFLOAT3 camera_focus = { 0.0f,1.0f,0.0f };
	...
	//카메라가 여러번 회전을 하게 되면 
	// 누적된 실수 연산의 부정확성 때문에 카메라의 로컬 xyz 축이 서로 직교하지 않을 수 있음 
	//카메라의 로컬 x-축(Right), y-축(Up), z-축(Look)이 서로 직교하도록 재생성
	...
	// 3인칭 카메라에서 카메라가 바라보는 지점을 설정
	// 일반적으로 플레이어를 바라보도록 설정
	virtual void SetLookAt(XMFLOAT3& xmf3LookAt) { }

	//절두체(월드 좌표계)를 생성
	void GenerateFrustum();
	//바운딩 박스(OOBB, 월드 좌표계)가 절두체에 포함되는 가를 검사
	bool IsInFrustum(BoundingOrientedBox& xmBoundingBox);
};

class Stone_Camera : public CCamera
{
public:
	Stone_Camera(CCamera* pCamera);
	virtual ~Stone_Camera() { }
	virtual void Rotate(float fPitch = 0.0f, float fYaw = 0.0f, float fRoll = 0.0f);

};

class TOP_Camera : public CCamera
{
public:
