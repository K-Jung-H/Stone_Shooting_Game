...
void CCamera::Update_Shader_Resource(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	//================================= 임시 변수의 내용을 리소스를 매핑한 포인터를 통해 전달

	CB_CAMERA_INFO* pcbMappedCamera = (CB_CAMERA_INFO*)(m_pMapped_camera_info);
	::memcpy(&pcbMappedCamera->m_xmf4x4View, &xmf4x4View, sizeof(XMFLOAT4X4));
	::memcpy(&pcbMappedCamera->m_xmf4x4Projection, &xmf4x4Projection, sizeof(XMFLOAT4X4));
	::memcpy(&pcbMappedCamera->m_xmf3Position, &m_xmf3Position, sizeof(XMFLOAT3));
...
Stone_Camera::Stone_Camera(CCamera* pCamera) : CCamera(pCamera)
{
	m_nMode = STONE_CAMERA;
	m_xmf3Up = XMFLOAT3(0.0f, 1.0f, 0.0f);
	m_xmf3Right.y = 0.0f;
	m_xmf3Look.y = 0.0f;
...
void Stone_Camera::Rotate(float x, float y, float z)
{
...
TOP_Camera::TOP_Camera(CCamera* pCamera) : CCamera(pCamera)
{
	m_nMode = TOP_VIEW_CAMERA;
	if (pCamera)
	{
...
void TOP_Camera::Rotate(float x, float y, float z)
{
	if (x != 0.0f)
	{
...
void TOP_Camera::SetLookAt(XMFLOAT3& xmf3LookAt)
{
	//현재 카메라의 위치에서 플레이어를 바라보기 위한 카메라 변환 행렬을 생성한다.
	XMFLOAT4X4 mtxLookAt = Matrix4x4::LookAtLH(m_xmf3Position, xmf3LookAt, GetUpVector());
...
CThirdPersonCamera::CThirdPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		...
		한다. 이것은 스페이스-쉽 카메라의 로컬 y-축 벡터가 어떤 방향이든지 3인칭 카메라(대부분 사람인 경우)의 로컬 y축
		벡터가 월드좌표의 y-축이 되도록 즉, 똑바로 서있는 형태로 설정한다는 의미이다. 그리고 로컬 x-축 벡터와 로컬
		...
		if (pCamera->GetMode() == STONE_CAMERA)
		{
			m_xmf3Up = XMFLOAT3(0.0f, 1.0f, 0.0f);
			m_xmf3Right.y = 0.0f;
			m_xmf3Look.y = 0.0f;
